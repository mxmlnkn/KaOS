...
main:
	...
	and al,11101111b	;lösche das Bit, das wenn es gesetzt ist die Tastatur deaktiviert.
	...
	call kb.checkcmd	;warte bis es angenommen wurde -> nun wartet der KC auf das Command-Byte am Port 60h(Datenport des 64h)
		pop ax			;stelle nun das mit 20h erhaltene Byte wieder her um es zu modifizieren und neu zu setzen
	and al,11101101b	;lösche das Bit 1, bei dessem setzen der IRQ12 immer angesprungen wird, sobald Bit5 im 64h-Status-Byte gesetzt ist
						;lösche auch noch Bit 5, da bei manchen PCs anscheined ein falsches command-byte geliefert wird.. und wenn bit 5 gesetzt ist,
						;wird der Keyboard-streaming-mode sozusagen beendet..
	out 60h,al			;und schicke das Command-Byte an den Daten-Port 60h, durch den Befehl 60h wird dieses neu eingelesen
	call kb.checkcmd	;und warte, bis das Byte vollständig angenommen wurde
	; movzx eax,al
...
.start:
cmp byte[actualb],0		;überprüfe, ob der Counter 0 ist (Zähler beginnt mit 0!)
jne .not_first_byte		;wenn nicht, hole schoneinmal Byte 1 nicht ab und springe somit darüber
	...
	call kb.read.mouse		;Byte #1 nach al holen 
		jnz .irq12_end		;wenn alles klar ging und das Byte geholt wurde, springe zum Ende, ohne den Byte-counter erhöht zu haben
	mov [status],al			;das Byte in Status speichern, damit es die Prozeduren von int33h nutzen können
	inc byte[actualb]		;erhöhe den Counter für das aktuelle Byte um 1, da es ja erflogreich abgeholt wurde
	jmp short .irq12_end	;beende den Interrupt, da beim nächsten Byte dieser IRQ sowieso nocheinmal aufgerufen wird

.not_first_byte:	
cmp byte[actualb],1			;teste ob der Counter 1 ist (Zähler beginnt mit 0!)
jne .not_second_byte		;wenn nicht, dann ist es auch nicht das zweite Byte, was wir jetzt holen müssen
	...
	call kb.read.mouse			;Byte #2 aus dem Output-Buffer der Maus holen
		jnz .irq12_end			;wenn dabei was schief ging, beende den irq
	...
	jnz .sign_bit_x				;und springe zu dem Teil, der subtrahiert statt addiert
		add [x],ax				;ansonsten addiere zur aktuellen Position die Bewegung -> da ah ja 0 sein muss, wenn der Programmzeiger hierhergekommen ist, geht das
		jmp short .sign_x		;und überspringe das Abziehen
	.sign_bit_x:
		...
		sub [x],ax				;und ziehe den Betrag des Movements der aktuellen Position ab -> da ah ja 0 sein muss, wenn der Programmzeiger hierhergekommen ist, geht das
	.sign_x:
	inc byte[actualb]		;erhöhe den Counter für das aktuelle Byte um 1, da es ja erflogreich abgeholt wurde
	jmp short .irq12_end	;beende den Interrupt, da beim nächsten Byte dieser IRQ sowieso nocheinmal aufgerufen wird

.not_second_byte:
cmp byte[actualb],2			;teste ob der Byte-Counter 2 ist (Zähler beginnt mit 0!)
jne .not_third_byte			;wenn nicht, dann muss es das 4. Byte sein, was wir abholen müssen also springe dahin
	...
	jnz .sign_bit_y				;und springe zu dem Teil, der subtrahiert statt addiert
		add [y],ax				;ansonsten addiere zur aktuellen Position die Bewegung -> da ah ja 0 sein muss, wenn der Programmzeiger hierhergekommen ist, geht das
		jmp short .sign_y		;und überspringe das Abziehen
	.sign_bit_y:
		not al					;negiere y-coord, damit der overflow aufgehoben wird, der ensteht, wenn man 0-1 rechnet
		inc al					;da aber -1 = FFh was genoted 0 wäre, addiere nocheinmal 1
		sub [y],ax				;und ziehe den Betrag des Movements der aktuellen Position ab -> da ah ja 0 sein muss, wenn der Programmzeiger hierhergekommen ist, geht das
	.sign_y:
	
	cmp byte[mouseid],0			;prüfe ob die MouseID 0 ist, denn wenn nciht, dann werden 4 Bytes gesendet, egal ob mouseid 3 o. 4
	jnz .four_byte_packet		;falls nun ein 4. Byte benötigt wird, springe zu Code, der den Byte-Counter nicht wieder auf 0 stellt
		...
		jmp short .irq12_end		;beende nun den Interrupt
	.four_byte_packet:
	inc byte[actualb]		;erhöhe den Counter für das aktuelle Byte um 1, da es ja erflogreich abgeholt wurde
	jmp short .irq12_end	;beende den Interrupt, da beim nächsten Byte dieser IRQ sowieso nocheinmal aufgerufen wird
...
.not_third_byte:
	...
	call kb.read.mouse		;hole Byte #4 aus dem Output-Buffer der Maus
		jnz .irq12_end		;wenn dabei was schief ging, beende den irq
